...
const test_exception = (function () {
  ...
  let resultingException: any | undefined;
  try {
    globalThis.require();
  } catch (anException) {
  ...
  if (resultingException) {
    globalThis.assert.strictEqual(resultingException.message, 'Error during Init');
...

{
  ...
  const returnedError = test_exception.returnException(throwTheError);
  globalThis.assert.strictEqual(returnedError, theError);
  ...
  globalThis.assert.strictEqual(exception_pending, true,
  ...
  //   globalThis.assert.strictEqual(returnedError, undefined,
...
//     caughtError = anError;
//   }
//   globalThis.assert.strictEqual(caughtError, undefined,
...
//   const exception_pending = test_exception.wasPending();
//   globalThis.assert.strictEqual(exception_pending, false,
