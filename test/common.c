                         ...
                         NAPIStatus expected_status, NAPIStatus actual_status)
{
    ...
    NAPI_CALL_RETURN_VOID(
        env, napi_create_string_utf8(env, (actual_status == expected_status ? expected_message : napi_message_string),
...
void add_last_status(NAPIEnv env, const char *key, NAPIValue return_value)
{
    ...
    NAPI_CALL_RETURN_VOID(
        env, napi_create_string_utf8(env, (p_last_error->errorMessage == NULL ? "napi_ok" : p_last_error->errorMessage),
                                     NAPI_AUTO_LENGTH, &prop_value));
