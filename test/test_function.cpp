...
static NAPIValue TestCreateFunctionParameters(NAPIEnv env,
    ...
    status = napi_create_function(nullptr,
                                  "TrackedFunction",
                                  NAPI_AUTO_LENGTH,
...
static NAPIValue TestFunctionName(NAPIEnv /*globalEnv*/, NAPICallbackInfo /*info*/) {
    return nullptr;
}
...
TEST_F(Test, TestFunction) {
    ...
    ASSERT_EQ(napi_create_function(
            globalEnv, nullptr, NAPI_AUTO_LENGTH, TestCallFunction, nullptr, &fn1), NAPIOK);

    NAPIValue fn2;
    ASSERT_EQ(napi_create_function(
            globalEnv, "Name", NAPI_AUTO_LENGTH, TestFunctionName, nullptr, &fn2), NAPIOK);

    NAPIValue fn3;
    ASSERT_EQ(napi_create_function(
            globalEnv, "Name_extra", -1, TestFunctionName, nullptr, &fn3), NAPIOK);

    NAPIValue fn4;
    ASSERT_EQ(napi_create_function(globalEnv,
                                   "MakeTrackedFunction",
                                   NAPI_AUTO_LENGTH,
                                   MakeTrackedFunction,
                                   nullptr,
    ...
    ASSERT_EQ(napi_set_named_property(globalEnv,
                                      exports,
                                      "MakeTrackedFunction",
                                      fn4), NAPIOK);
    ...
    ASSERT_EQ(NAPIRunScriptWithSourceUrl(globalEnv,
                                         "(()=>{\"use strict\";function l(l){return l+1}globalThis.assert.strictEqual(globalThis.addon.TestCall((function(){return 1})),1),globalThis.assert.strictEqual(globalThis.addon.TestCall((function(){return null})),null),globalThis.assert.strictEqual(globalThis.addon.TestCall(l,1),2),globalThis.assert.strictEqual(globalThis.addon.TestCall((function(a){return l(a)}),1),2),globalThis.assert.strictEqual(globalThis.addon.TestName.name,\"Name\"),globalThis.assert.strictEqual(globalThis.addon.TestNameShort.name,\"Name_extra\")})();",
                                         "https://www.didi.com/test_function.js",
                                         &result), NAPIOK);
}