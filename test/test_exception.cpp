...
static void finalizer(NAPIEnv env, void * /*data*/, void * /*hint*/) {
    NAPI_CALL_RETURN_VOID(env,
                          napi_throw_error(env, nullptr, "Error during Finalize"));
}
...
static NAPIValue createExternal(NAPIEnv env, NAPICallbackInfo /*info*/) {
    ...
    NAPI_CALL(env,
              napi_create_external(env, nullptr, finalizer, nullptr, &external));
...
TEST(TestException, RequireThrowException) {
    ...
    ASSERT_EQ(NAPIRunScriptWithSourceUrl(globalEnv,
                                         "(()=>{\"use strict\";var r=new Error(\"Some error\"),t=function(){var r;try{globalThis.require()}catch(t){r=t}if(r)return globalThis.assert.strictEqual(r.message,\"Error during Init\"),r.binding}(),n=function(){throw r},e=t.returnException(n);globalThis.assert.strictEqual(e,r),globalThis.assert.throws((function(){t.allowException(n)}),(function(t){return t===r}));var i=t.wasPending();globalThis.assert.strictEqual(i,!0,`Exception not pending as expected, .wasPending() returned ${i}`)})();",
                                         "https://www.didi.com/test_exception_test.js",
                                         &result), NAPIOK);
}